...
import {clearErrors, getVisaCardDetails} from "../../actions/visaAction"
...
const ApplyVisaCard = () => {
    ...
    const {visaCard, loading, error} = useSelector((state) => state.VisaCardDetails)
    useEffect(() => {
        if(error){
          alert.error(error)
          dispatch(clearErrors())
        }
        dispatch(getVisaCardDetails(params.id))
            ...
            <div className="row justify-content-between align-items-center mt-5">
              <div className="col-lg-8">
                <div className="card p-3 Visa-Type-card">
                  ...
                  >
                    <option value={visaCard.name} selected> {visaCard.name}</option>
                  </select>
